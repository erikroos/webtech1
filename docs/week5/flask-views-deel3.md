...
Drie stappen: object maken, toevoegen aan sessie, committen. Net als in het vorige deel gebeurt alles binnen `with app.app_context():` — dat geldt ook voor alle volgende voorbeelden in dit deel (bekijk `basic_CRUD.py` om te zien hoe dat er compleet uitziet).
