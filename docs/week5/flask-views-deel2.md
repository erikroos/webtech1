...
!!! warning "Application context"
    Let op het `with app.app_context():`-blok. SQLAlchemy heeft toegang nodig tot de Flask-configuratie (waar staat de database?), en die is buiten een draaiende Flask-applicatie alleen beschikbaar binnen zo'n *application context*. Voer daarom in losse scripts álle databaseoperaties uit binnen dit blok — anders krijg je de foutmelding *Working outside of application context*.

### Records toevoegen

Binnen hetzelfde `with`-blok:
